...
def setup_args():
    ...
    parser.add_argument(
        ...
        default=np.log(2.0) / 60.0,
    )  # corresponding to size doubling time = 1 hour, in /min
    parser.add_argument(
        "--alpha_sd",
        type=float,
        help="Equilibrium standard deviation for alpha (in [/min])",
        default=np.log(2.0) / (10.0 * 60.0),
    )  # 1/10 of default target, in /min
    parser.add_argument(
        "--alpha_autocorr",
        type=float,
    ...
    parser.add_argument(
        "--birth_size_mean", type=float, help="Initial mean birth size", default=1.0
    )
    parser.add_argument(
        "--birth_size_mode", type=str, help="[fixed, lognormal]", default="fixed"
    )
    parser.add_argument(
        "--birth_size_relative_fluctuations",
        type=float,
    ...
    parser.add_argument(
        "--start", type=float, help="Time at which simulation starts", default=0.0
    )
    parser.add_argument(
        "--stop", type=float, help="Time at which simulation stops", default=180.0
    )
    parser.add_argument(
        "--period",
        type=float,
        help=("Time period between two consecutive time-lapse" " acquisitions"),
        default=5.0,
    )
...
def main():
    ...
    # otherwise read default
        ...
        spring = (
            1.0 / args.alpha_autocorr
        )  # spring constant (brings back random walker to target value)
        noise_intensity = (
            2.0 * spring * (args.alpha_sd) ** 2
        )  # noise intensity for random walk
        ...
        birthsizeParams = SampleInitialSize(
            birth_size_mean=args.birth_size_mean,
