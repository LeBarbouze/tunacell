...
class FilterTree(FilterGeneral):
    ...
    _type = "TREE"
    _INTERNAL_BASENAME = "filter_trees.txt"
...
class FilterTreeDepth(FilterTree):
    ...
    def func(self, tree):
        ...
        if isinstance(tree, treelib.Tree):
            boo = bounded(
                tree.depth(), lower_bound=self.lower_bound, upper_bound=self.upper_bound
            )
        else:
...
class FilterTreeTimeIntersect(FilterTree):
    ...
    def func(self, tree):
        ...
        return intersect(
            values, lower_bound=self.lower_bound, upper_bound=self.upper_bound
        )
