...
def test_division_params():
    ...
    growth_rates = np.log(2.0) / (60.0 * np.array([20.0, 33.3, 47.7, 60.0, 1001.0]))
    ...
    for item in itertools.product(lambdas, sizes, modes, flucts):
        ...
        birth_sizes = np.linspace(div_size / 10.0, div_size, 5, endpoint=False)

        for bs, mu in itertools.product(birth_sizes, growth_rates):
            assert dp.rv(birth_size=bs, growth_rate=mu) > 0.0
...
def test_sample_initial_size():
    ...
    for size, fluct in itertools.product(sizes, flucts):
        bs = SampleInitialSize(
            birth_size_mean=size,
