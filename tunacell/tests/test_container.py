...
def test_attributes(container):
    if isinstance(container.abspath, pathlib.Path):
        assert str(container.abspath.absolute()) == os.path.join(
            os.path.abspath(path_fake_exp), "containers", "container_01.txt"
        )
    else:
        assert container.abspath == os.path.join(
            os.path.abspath(path_fake_exp), "containers", "container_01.txt"
        )
...
def test_container_data(container):
    ...
    assert container.data["time"][0] == 0.0
    assert container.data["value"][0] == 1.0
