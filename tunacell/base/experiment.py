...
class Experiment(object):
    ...
    def __init__(self, path=".", filetype=None, filter_set=None, count_items=False):
        self.abspath = None
        self.label = None
        ...
        self.path_internals = (
            self.path_root / "internals"
        )  # defines path to internals folder
    ...
    def analysis_path(self):
        ...
        index, filterset_path = analysis.get_filter_path(
            analysis_path, self.fset, write=True
        )
    ...
    def count_items(self, independent=True, seed=None, read=True):
        ...
        try:
            if read:
                ...
                i, filter_path = analysis.get_filter_path(
                    analysis_path, self.fset, write=False
                )
            ...
            text.MissingFileError,
            text.MissingFolderError,
            text.CorruptedFileError,
        ):
    ...
    def iter_colonies(self, filter_for_colonies="from_fset", size=None, shuffle=False):
        ...
        if size is not None:
            ...
            for container in self.iter_containers(shuffle=shuffle):
                for colony in container.iter_colonies(
                    filter_for_colonies=colony_filter, shuffle=shuffle
                ):
                    yield colony
                    count += 1
                    if count >= size:
    ...
    def iter_lineages(self, filter_for_lineages="from_fset", size=None, shuffle=False):
    ...
    def raw_text_export(self, path=".", metadata_extension=".yml"):
        ...
        if not os.path.exists(data_path):
            ...
            msg = (
                "Container files already exists.\n"
...
def count_items(exp, independent_decomposition=True, seed=None):
    ...
    for container in tqdm(
        ...
        for colony in container.iter_colonies(filter_for_colonies="from_fset"):
            colonies += 1
            for lineage in colony.iter_lineages(
                independent=independent_decomposition, seed=seed
            ):
                lineages += 1
