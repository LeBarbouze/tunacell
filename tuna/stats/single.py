...
#def _export_to_text(obj, user_path=None):
#    """Export results to text files.
#
#    This function sets up directory structure and a call the write method
#    for each conditioned observable. obj must have a getitem defined over
#    filter representations in cset
#
#    Parameter
#    ---------
#    obj : one of {Univariate, StationaryUnivariate} instance
#        must have .obs, .parser, .cset attributes, a getitem method
#        defined over repr of each filters in cset (obj['master'] must point
#        to a Conditioned object), and each of the Conditioned object must
#        have a write_text() method defined.
#    user_path : str
#        user defined path
#
#    Notes
...
#    """
#    obs = obj.obs
#    fset = obj.parser.fset
#    cset = obj.cset
#    exp = obj.parser.experiment
#    # SET FOLDERS FOR OBSERVABLE
#    # if exporting Univariate and path to obs exists -> cache, set
#    try:
#        obspath = text.get_analysis_single(exp, fset, obs,
#                                           user_abspath=user_path)
#        if isinstance(obj, Univariate):
#            warnings.warn('Previous folder moved to cache')
#            foldup, bn = os.path.split(obspath)
#            cached = os.path.join(foldup, '.' + bn)
#            if os.path.exists(cached):
#                warnings.warn('Previous existing cached folder deleted')
#                shutil.rmtree(cached)
#            shutil.move(obspath, cached)
#            os.makedirs(obspath)
#    # maybe it does not exist at all -> create
#    except text.MissingFolderError:
#        obspath = text.set_analysis_single(exp, fset, obs,
#                                           user_abspath=user_path)
#
#    with open(os.path.join(obspath, 'observable.txt'), 'w') as f:
#        f.write('{}\n\n{}\n\n{}\n\n{}'.format(repr(obs),
#                                              str(obs),
#                                              obs.as_latex_string(),
#                                              obs.as_string_table()))
#    # write Indexify object when exporting Univariate object
#    if isinstance(obj, Univariate):
#        with open(os.path.join(obspath, 'indexify.txt'), 'w') as f:
#            f.write(repr(obj.indexify))
#    elif isinstance(obj, StationaryUnivariate):
#        stat_params_file = os.path.join(obspath, 'stationary_params.tsv')
#        if not os.path.exists(stat_params_file):
#            with open(stat_params_file, 'w') as f:
#                f.write('label\ttmin\ttmax\tadjust_mean')
#        with open(stat_params_file, 'a') as f:
#            f.write('\n{}\t{}\t{}\t{}'.format(obj.label, obj.tmin, obj.tmax,
#                                              obj.adjusy_mean))
#
#    # WRITE MASTER
#    master_path = os.path.join(obspath, 'master')
#    if not os.path.exists(master_path):
#        os.makedirs(master_path)
#    obj['master'].write_text(path=os.path.join(obspath, 'master'))
#    # WRITE CONDITIONS
#    for index, cdt in enumerate(cset, start=1):
#        cdt_label = 'condition_{:02d}'.format(index)
#        cdt_path = os.path.join(obspath, cdt_label)
#        if not os.path.exists(cdt_path):
#            os.makedirs(cdt_path)
#        # write text file to save what filter is used
#        with open(os.path.join(cdt_path, cdt_label + '.txt'), 'w') as f:
#            f.write('{}\n\n{}'.format(repr(cdt), str(cdt)))
#        cdt_repr = '{}'.format(repr(cdt))
#        obj[cdt_repr].write_text(path=cdt_path)
#    return
#
#
#def _import_from_text(obj, user_path=None):
#    """Set instance from text files.
#
#    This function parse folders to read data. Object <obj> must be initialized
#    with appropriate .obs, .parser, and .cset attributes.
#
#    Parameters
#    ----------
#    one of {Univariate, StationaryUnivariate} instance
#        must have .obs, .parser, .cset attributes, a getitem method
#        defined over repr of each filters in cset (obj['master'] must point
#        to a Conditioned object), and each of the Conditioned object must
#        have a read_text() method defined.
#    user_path : str
#        user defined path
#
#    Raises
#    ------
#    UnivariateIOError : when analysis folder is not found
#    """
#    obs = obj.obs
#    fset = obj.parser.fset
#    exp = obj.parser.experiment
...
#        obspath = text.get_analysis_single(exp, fset, obs,
#                                           user_abspath=user_path)
#    except text.MissingFolderError as mf:
#        print(mf)
#        raise UnivariateIOError(mf)
#    if isinstance(obj, Univariate):
#        with open(os.path.join(obspath, 'indexify.txt'), 'r') as f:
#            line = f.readline()
#        obj.indexify = eval(line)
#    elif isinstance(obj, StationaryUnivariate):
#        with open(os.path.join(obspath, 'stationary_params.txt'), 'r') as f:
#            for line in f.readlines():
#                key, sval = line.rstrip().split(',')
#                if key == 'tmin':
#                    if sval != 'None':
#                        obj.tmin = float(sval)
#                elif key == 'tmax':
...
#    # GET MASTER
#    obj['master'].read_text(path=os.path.join(obspath, 'master'))
#    # GET ALL CONDITIONS
#    for index, cdt in enumerate(cset, start=1):
#        cdt_label = 'condition_{:02d}'.format(index)
#        cdt_repr = '{}'.format(repr(cdt))
#        cdt_path = os.path.join(obspath, cdt_label)
#        obj[cdt_repr].read_text(path=cdt_path)
#    return
